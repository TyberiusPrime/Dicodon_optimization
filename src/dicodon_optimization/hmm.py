...
class DegenerateHMM:
    ...
    def __init__(self, states, transition_scores):
        ...
        for (from_state, to_state) in self.transition_scores:
            if not from_state in self.states:
                raise ValueError("There was a transition from %s to %s, but %s was not in 'states'"
        ...
        for key in self.transition_scores:
            if self.transition_scores[key] > 0:
                ...
                raise ValueError("Negative scores are not allowed (was in transition %s -> %s)" % key)
    ...
    def viterbi(self, observed_sequence):
        ...
        for t in xrange(0, len(observed_sequence)):
            ...
            V.append({})
        path = {} #for the position we have reached in observed_sequence, the survivor paths for each possible state
        ...
        for t in xrange(1, len(observed_sequence)):
            ...
            for to_state, emission in self.states.items(): #for each state we could potentially end up
                if emission == observed_sequence[t]: #this is valid transition, we can end up here
                    ...
                    for from_state, emission in self.states.items():
                        ...
                        score_here = transition_score + score_for_being_in_from_state #+ in log is * in the normal scale
                        if score_here > survivor_score: #it's best (so far) to have come from from_state!
                            survivor_score = score_here
                            survivor_state = from_state
                            any_survivors = True #this Hmm can have produced this sequence
                    if survivor_state is not None: #there is no way we can have ended up in to_state. But there might be other states with the right emission
            ...
            if not any_survivors:
                raise ModelSequenceMismatchException("This HMM can't have generated this sequence in position %i" % t)
            path = new_path
        final_scores = V[-1]
        ...
        if maximum_final_state is None:
            raise ModelSequenceMismatchException("This HMM can't have generated this sequence in position %i" % len(observed_sequence))
