...
def optimize_dicodon_usage(amino_acid_sequence, 
        dicodon_score_dict):
    ...
    dicodon_plus_score = []
    for tt in xrange(0, len(state_sequence) - 1):
        dicodon = state_sequence[tt], state_sequence[tt + 1]
        score = dicodon_score_dict[dicodon] #must be in there, otherwise the viterbi should not have picked it.
        dicodon_plus_score.append((dicodon, score))
    return optimized_sequence, dicodon_plus_score

def dicodon_count_from_sequences(list_of_dna_sequences, ignore_errors = False):
    ...
    for sequence in list_of_dna_sequences:
        if len(sequence) % 3 != 0:
            if ignore_errors:
                print ValueError("a sequence was not a multiple of 3 bases - not a coding sequence %s" % sequence)
                continue
            else:
                raise ValueError("a sequence was not a multiple of 3 bases - not a coding sequence %s" % sequence)
        elif len(sequence) < 6:
            if ignore_errors:
                print ValueError("A sequence was to short (did not contain a single dicodon): %s" % sequence)
                continue
            else:
                raise ValueError("A sequence was to short (did not contain a single dicodon): %s" % sequence)
        sequence = sequence.lower()
        for start_pos in xrange(0, len(sequence), 3): #assumption: in frame sequence
            ...
            if not (codonA, codonB) in dicodon_frequency:
                dicodon_frequency[codonA, codonB] = 0
...
def codon_count_from_sequences(list_of_dna_sequences):
    ...
    for sequence in list_of_dna_sequences:
        if len(sequence) % 3 != 0:
            raise ValueError("a sequence was not a multiple of 3 bases - not a coding sequence")
        sequence = sequence.lower()
        for start_pos in xrange(0, len(sequence), 3): #assumption: in frame sequence
            codon = sequence[start_pos: start_pos + 3]
            if not codon in codon_frequency:
                codon_frequency[codon] = 0
            codon_frequency[codon] += 1
    return codon_frequency

def dicodon_score_dict_from_sequences(list_of_dna_sequences, ignore_errors = False):
    #first, count di codons.
    dicodon_counts = dicodon_count_from_sequences(list_of_dna_sequences, ignore_errors)
    result = {}
        ...
        for secondAA in amino_acids: #we find all paths that lead to a given AA
            ...
            for codonB in codons_for_second:
                ...
                if dicodon in dicodon_counts: #we observed this dicodon:
                    dicodons_for_this_transition.append((dicodon, dicodon_counts[dicodon]))
...
def score(dna_sequence, dicodon_score_dict):
    """score a coding DNA sequence dicodon_score_dict"""
    result = []
    dna_sequence = dna_sequence.lower()
    for start_pos in xrange(0, len(dna_sequence), 3): #assumption: in frame sequence
...
def translate_to_aa(dna_sequence):
    if len(dna_sequence) % 3 != 0:
        raise ValueError("Not a coding DNA sequence, lenght is not divisible by 3, was %i" % len(dna_sequence))
    aa = ''
    dna_sequence = dna_sequence.lower()
    for start in xrange(0, len(dna_sequence), 3):
